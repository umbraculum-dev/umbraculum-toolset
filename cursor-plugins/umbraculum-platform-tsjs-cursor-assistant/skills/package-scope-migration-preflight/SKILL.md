...
14. **MANDATORY pre-push: run `bash <REPO_ROOT>/scripts/ci-parity-check.sh`** — this is the **operational answer** to the user's escalation 2026-05-19 (*"why can't we catch them locally? we must understand how to catch them locally"*) after slot 7's CI failed three jobs (typecheck, web-lint, check-module-readmes) that all reported green on the developer's host. The script reproduces the three static-analysis CI jobs (`docs-readmes`, `typecheck`, `web-lint`) against a clean `git archive HEAD` snapshot — eliminating the three local-vs-CI divergence mechanisms documented in `docs/design/brewery-scope-migration-plan.md` §6.7: (a) host filesystem has gitignored files CI's checkout doesn't (e.g. `DEVELOPMENT.md`); (b) developer host has stale `apps/web/e2e/node_modules` from prior manual installs, bind-mounted into local docker reproductions, that masks the nested-workspace-not-in-root-glob trap (`apps/web/e2e` is NOT matched by `workspaces: ["apps/*", "services/*", "packages/*"]`); (c) stale `<workspace>/node_modules` shadowing changes the rule-firing behavior of type-aware ESLint rules like `@typescript-eslint/require-await`. Bounded ~2 minutes. Run as the LAST step before `git push` on every slot commit. If the script reports any FAIL, fix locally and re-run before pushing — do NOT push expecting CI to "tell us what's wrong". **Surfaced as a hard recommendation during CI hygiene fix #2 (post-slot-7).**
