...
- `<REPO_ROOT>` — absolute monorepo root (e.g. `/path/to/umbraculum-dev`).
...
**Lockfile:** T2 validate always runs **`npm run check:lockfiles`** (forbidden `services/api/package-lock.json` + root + web sync in `node:20-slim`) before any ci-parity job. When any committed **`package-lock.json`** changes, agents must run skill **`monorepo-lockfile-gate`**, test, and commit lock + `package.json` together (rule **`80-monorepo-lockfile-agent-gate`**).
