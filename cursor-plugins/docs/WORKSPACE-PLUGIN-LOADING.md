...
> [!CAUTION]
> **Why this matters — context budget.** Each plugin ships many `alwaysApply: true`
> rules plus skills and subagents. If you load the **wrong** plugins for a workspace,
> or load **all** plugins globally, Cursor injects unrelated guidance into every
> prompt. Agents then **lose precision** on the rules that matter, apply the wrong
> stack family, and maintainers report "the agent ignored the rule" when the real
> issue is **plugin overload**. Match the [pairing matrix](#what-you-get) exactly —
...
**Must NOT appear** in umbraculum-dev: extra toolset plugins beyond the three rows above (e.g. `umbraculum-openplc-python-cursor-assistant`).

Every hook-loaded workspace includes **`umbraculum-toolset-common`** (DEVELOPMENT-LOCAL gate, Skill Contract, commit-message ticket prefix, `generate-development-local` skill).

### Source repositories (clone once)

| Purpose | Path on disk |
|---|---|
| Umbraculum plugins (four folders in this repo) | `/path/to/umbraculum-toolset/cursor-plugins/` |
...
- **Empty** `~/.cursor/plugins/local/` for umbraculum plugins (see step 3).
...
**2. Create the discriminator script** — for **umbraculum-dev contributors**, copy `cursor-plugins/scripts/register-workspace-plugins.umbraculum-dev.example.sh`. For **multi-workspace maintainers** (umbraculum-dev + OpenPLC sister repo), copy `cursor-plugins/scripts/register-workspace-plugins.example.sh`. Install to `~/.cursor/hooks/register-workspace-plugins.sh` (or use the [§3 skeleton](#skeleton-homecursorhooksregister-workspace-pluginssh) inline), then:
...
`~/.cursor/plugins/local/` should contain **no** umbraculum plugin folders. Anything left there loads **globally** in every workspace, regardless of the hook.
...
```bash
echo '{"workspace_roots":["/path/to/umbraculum-dev"]}' \
  | ~/.cursor/hooks/register-workspace-plugins.sh
```

Expect `pluginPaths` with `umbraculum-toolset-common`, `umbraculum-node-react-cursor-assistant`, and `umbraculum-platform-tsjs-cursor-assistant` only.

### Legacy rollback (avoid unless abandoning hook)

```bash
# 1. Comment out workspaceOpen in ~/.cursor/hooks.json
# 2. Global rsync (loads all umbraculum plugins everywhere):
...
> [!CAUTION]
> **Marketplace listings ≠ install every listing everywhere.** Each plugin
> targets a different workspace type (umbraculum-platform, generic TS/JS,
> OpenPLC). Enable **only the pairing row for the workspace you have open** —
> otherwise you recreate global context noise and wrong guardrails.
> See [§0 pairing matrix](#what-you-get).
...
2. Enable it **for the target workspace only** — use the workspace filter chip in
   **Settings → Plugins**. Avoid "Add for Myself" / all-workspaces enablement
   unless you deliberately want that plugin in every project (usually you do not).
...
3. **Settings → Hooks** — confirm `workspaceOpen` is registered and the last run's `pluginPaths` match the workspace (three paths for umbraculum-dev, two for OpenPLC brewery, one for generic repos).
...
5. **Agent smoke** — new chat in umbraculum-dev: witness / ci-parity rules present; no unrelated domain-plugin guardrails.
